...
```{r}
inchi_url <- paste(api,"cid/",compoundID,"/property/inchi/TXT",sep = "")
...
```{r}
api <- "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/";
SS_url <- paste(api,'fastsimilarity_2d/cid/',compoundID, '/cids/JSON?Threshold=95',sep = "");
...
```{r}
CIDs1_df <- do.call(what = "rbind",
                    args = lapply(CIDs1_ls, as.data.frame)) 
#converts the list of lists to a dataframe
#lapply applies its second argument (as.data.frame) to every element of the list in the first argument (result1_ls)
#rbind binds all the dataframes together
#do.call feeds in all the arguments (dataframes to be binded together) to the rbind command 
...
```{r message=FALSE, warning=FALSE}
png("Data/Similarity_plot_save.png", width = 2096, height = 2096); #Initialize the results png

par(mfrow=c(5,5),mar=c(.1,.1,.1,.1)) #setup plot parameters

for(CID in short_CIDs[1:25]){
  #define the url
...
for (CID in hit_CIDs_results) {
  ...
  api = 'https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/';
  CID_InChI_url = paste(api,'cid/',toString(CID),'/property/InChI/TXT',sep = "");
  CID_CanSMI_url = paste(api,'cid/',toString(CID),'/property/CanonicalSMILES/TXT',sep="");
  CID_MW_url = paste(api,'cid/',toString(CID),'/property/MolecularWeight/TXT',sep="");
  CID_IUPACName_url = paste(api,'cid/',toString(CID),'/property/IUPACName/TXT',sep="");
  CID_HeavyAtomCount_url = paste(api,'cid/',toString(CID),'/property/HeavyAtomCount/TXT',sep="");
  CID_CovalentUnitCount_url = paste(api,'cid/',toString(CID),'/property/CovalentUnitCount/TXT',sep="");
  CID_Charge_url = paste(api,'cid/',toString(CID),'/property/Charge/TXT',sep="");
...
```{r}
include_graphics("Data/Smarts_plot_save.png")
```
